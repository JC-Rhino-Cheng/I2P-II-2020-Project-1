...
int flag_is_memory_idx_available[MAXLEN];
...
typedef enum {
	...
	INCDEC, ASSIGN,
	LPAREN, RPAREN,
	DISCARDED_CZ_PROCESSED___before_not_assign,
	DISCARDED_CZ_PROCESSED___before_is_assign
} TokenSet;
...
int main() {
	...
	while (!flag_now_expr_error) {
		...
		for (int i = 0; i < MAXLEN; i++)flag_is_memory_idx_available[i] = 1;
...
void output_assembly(BTNode *root, BTNode *parent) {
	...
	if (root->data != ASSIGN && root->data != DISCARDED_CZ_PROCESSED___before_is_assign && root->data != DISCARDED_CZ_PROCESSED___before_not_assign)output_assembly(root->left, root);
	...
	if (root->data == ADDSUB || root->data == MULDIV || root->data == LOGICAL) {
		...
		if (root->left->data == ID || root->left->data == INT) {
			for (int i = 0; i < reg_max_len; i++) {
				if (flag_is_reg_idx_available[i]) {
					...
					for (int j = MAXLEN - 1; j >= 0; j--) {//開始從[255]->[254]->[253]...etc找尋可用的memory位置來存放r0、r1、r2。注意: reg的編號，和，memory的編號，以255互補
						if (flag_is_memory_idx_available[j]) {
							flag_is_memory_idx_available[j] = 0;
							left_reg_idx = 255 - j;
							break;
						}
					}
		...
		root->data = root->right->data = root->left->data = DISCARDED_CZ_PROCESSED___before_not_assign;//因為如果改成ID或者INT，之後明明已經輸出完畢了還是會因為判斷式子而進來。雖然他進來之後只會進行冗餘的指令，並不影響正確性，不過乾脆就不讓他進來，比較簡單，time clock cycle也比較俐落。
		...
		if (flag_is_memory_idx_available[255 - root->right->reg_num]) {//如果[255]是可用的，代表[255]是空的，代表r0是可以被釋放的
			flag_is_reg_idx_available[root->right->reg_num] = 1;
		}
		else if (!flag_is_memory_idx_available[255 - root->right->reg_num]) {//如果[255]是在工作狀態，代表[255]現在儲存的是原本的r0
			printf("MOV r%d [%d]\n", root->right->reg_num, 255 - root->right->reg_num);//把[255]的內容丟還給r0
			//r0不能被釋放掉，因為r0是要被換回原本的r0
			//但是要釋放掉已經完成工作的[255]
			flag_is_memory_idx_available[255 - root->right->reg_num] = 1;
		}
		
	}
	else if (root->data == ASSIGN) {
		root->data = DISCARDED_CZ_PROCESSED___before_is_assign;

		//先找出左邊的變數的編號
		int idx_of_variable_of_root_of_left_subtree = 0;
		for (int i = 0; i < TBLSIZE; i++) {
		...
		int idx_of_variable_of_root_of_right_subtree = 0;
		for (int i = 0; i < TBLSIZE; i++) {
		...
		if (root->right->data == ID)printf("MOV r%d [%d]\n", root->right->reg_num, 4 * idx_of_variable_of_root_of_right_subtree);
		else if (root->right->data == INT)printf("MOV r%d %d\n", root->right->reg_num, root->right->val);
		root->right->data = DISCARDED_CZ_PROCESSED___before_is_assign;
		printf("MOV [%d] r%d\n", 4 * idx_of_variable_of_root_of_left_subtree, root->right->reg_num);
