...
typedef enum {
	...
	ADDSUB_MULDIV_LOGICAL_from_assignment_to_be_discarded
} TokenSet;
...
void output_assembly(BTNode *root) {
	...
	if (root->data == INT || root->data == ID) {
		...
		if (root->data == INT) {
			root->data = INT_from_assignment_to_be_discarded;//因為如果改成ID或者INT，之後明明已經輸出完畢了還是會因為判斷式子而進來。雖然他進來之後只會進行冗餘的指令，並不影響正確性，不過乾脆就不讓他進來，比較簡單，time clock cycle也比較俐落。
			root->reg_num = reg_idx;
			printf("MOV r%d %s\n", root->reg_num, root->lexeme);
		}
		else if (root->data == ID) {
			root->data = ID_from_assignment_to_ID_to_be_discarded;//因為如果改成ID或者INT，之後明明已經輸出完畢了還是會因為判斷式子而進來。雖然他進來之後只會進行冗餘的指令，並不影響正確性，不過乾脆就不讓他進來，比較簡單，time clock cycle也比較俐落。
			root->reg_num = reg_idx;
		...
		root->data = ADDSUB_MULDIV_LOGICAL_from_assignment_to_be_discarded;//因為如果改成ID或者INT，之後明明已經輸出完畢了還是會因為判斷式子而進來。雖然他進來之後只會進行冗餘的指令，並不影響正確性，不過乾脆就不讓他進來，比較簡單，time clock cycle也比較俐落。
		...
		if (root->right->data == ID_from_assignment_to_ID_to_be_discarded) {
			...
			root->data = ID_from_assignment_to_ID_to_be_discarded;//因為如果改成ID或者INT，之後明明已經輸出完畢了還是會因為判斷式子而進來。雖然他進來之後只會進行冗餘的指令，並不影響正確性，不過乾脆就不讓他進來，比較簡單，time clock cycle也比較俐落。
			root->right->data = INT_from_assignment_to_be_discarded;//因為如果改成ID或者INT，之後明明已經輸出完畢了還是會因為判斷式子而進來。雖然他進來之後只會進行冗餘的指令，並不影響正確性，不過乾脆就不讓他進來，比較簡單，time clock cycle也比較俐落。
