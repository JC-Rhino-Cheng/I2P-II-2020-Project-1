...
void output_assembly(BTNode *root) {
	...
	if (root->data != ASSIGN && root->data != DISCARDED_CZ_PROCESSED___before_is_assign && root->data != DISCARDED_CZ_PROCESSED___before_not_assign)output_assembly(root->left);
	...
	if (root->data == INT || root->data == ID) {
		...
		if (root->data == INT) {
			root->data = DISCARDED_CZ_PROCESSED___before_not_assign;//因為如果改成ID或者INT，之後明明已經輸出完畢了還是會因為判斷式子而進來。雖然他進來之後只會進行冗餘的指令，並不影響正確性，不過乾脆就不讓他進來，比較簡單，time clock cycle也比較俐落。
		...
		if (root->right->data == DISCARDED_CZ_PROCESSED___before_not_assign) {//等號右側未經處理前，不是個等號，代表代表目前的root不是連續等號，或者是連續等號但是是從右邊數過來的第一個等號
			printf("MOV [%d] r%d\n", 4 * idx_of_variable_of_root_of_left_subtree, root->right->reg_num);
			root->reg_num = root->right->reg_num;
			//不能釋放reg成available狀態
		}
		else if (root->right->data == DISCARDED_CZ_PROCESSED___before_is_assign) {
			printf("MOV [%d] r%d\n", 4 * idx_of_variable_of_root_of_left_subtree, root->right->reg_num);
			root->reg_num = root->right->reg_num;
			//不能釋放reg成available狀態
		}
